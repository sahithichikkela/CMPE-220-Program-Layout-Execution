...
void stack_push8(CPU *cpu, uint8_t value) {
    cpu->regs.SP--;                         // Decrement FIRST
    mem_write8(cpu, cpu->regs.SP, value);  // THEN write
}

uint8_t stack_pop8(CPU *cpu) {
    uint8_t value = mem_read8(cpu, cpu->regs.SP);  // Read FIRST
    cpu->regs.SP++;                                 // THEN increment
    return value;
}
...
// uint16_t stack_pop16(CPU *cpu) {
//     uint8_t high = stack_pop8(cpu);  // Changed order!
//     uint8_t low = stack_pop8(cpu);   // Changed order!
//     return (high << 8) | low;
// }
