...
bool parse_operand(Assembler *as, char *operand, uint8_t *mode, 
    ...
    // ★★★ CRITICAL FIX: Labels should use IMMEDIATE mode for jumps/calls ★★★
    *mode = MODE_IMMEDIATE;  // Changed from MODE_DIRECT
    if (isdigit(*operand) || *operand == '-') {
        *value = strtol(operand, NULL, 0);
    } else {
        // Label reference
...
bool parse_line(Assembler *as, char *line, bool first_pass) {
    ...
    // Special handling for MOV instruction (needs two register operands)
    if (opcode == OP_MOV && operand_str[0] != '\0') {
        // Parse source register
        char *space = strchr(operand_str, ' ');
        if (!space) {
            fprintf(stderr, "Error line %d: MOV requires two operands\n", as->line_number);
            return false;
        }
        
        *space = '\0';  // Split the string
        char *src_str = operand_str;
        char *dst_str = space + 1;
        while (*dst_str && isspace(*dst_str)) dst_str++;  // Skip whitespace
        
        // Parse source
        uint8_t src_mode, dst_mode;
        uint16_t src_val, dst_val;
        
        if (!parse_operand(as, src_str, &src_mode, &src_val, first_pass)) {
            return false;
        }
        if (!parse_operand(as, dst_str, &dst_mode, &dst_val, first_pass)) {
            return false;
        }
        
        // Both must be registers for MOV
        if (src_mode != MODE_REGISTER || dst_mode != MODE_REGISTER) {
            fprintf(stderr, "Error line %d: MOV requires register operands\n", as->line_number);
            return false;
        }
        
        // Emit instruction (only in second pass)
        if (!first_pass) {
            emit_byte(as, encode_instruction(opcode, MODE_REGISTER));
            emit_byte(as, src_val);  // Source register
            emit_byte(as, dst_val);  // Destination register
        } else {
            as->current_address += 3;  // 1 opcode + 2 register bytes
        }
        
        return true;
    }

    // Normal operand parsing for other instructions
    if (operand_str[0] != '\0') {
